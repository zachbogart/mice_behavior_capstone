...
def load_data(conditions_folder,start_frame=None,end_frame=None,ext='.zones.dict'):
    ...
    zones_masks = dict([(k,vidtools.mask_from_outline(vidtools.outline_from_polygon(v),shape)) for k, v in zones.items()])
...
def arm_entry(results_array, zones_order):
    ...
    for frame in range(len(results_array)-1):
        ...
        for zone in [4,6,7,8]: # 4,6,7,8 are the arms
            if results_array[frame,5] > 0: # 5 is the middle area. Arm antries can only occur if a mouse was present in the middle.
...
def time_in_arms(results_array,zones_order):
    ...
    for frame in range(len(results_array)-1):
        for i, zone in enumerate(range(4,9)):
            if np.argmax(results_array[frame]) == zone: #Mouse is counted to spend time in the area where most of it's outline is present.
                arm_residence[i] += 1
    arms = zones_order[4:9]
    return dict(zip(arms, arm_residence))

def fill_in_missing_outlines(mice_ols): #fills in position of mouse that disappeared due to immobility based on where he was before he disapeared
...
def calcPosition(shape,filled_in_mice_ols):
    '''
    ...
    for arm,arcs in arcs_in_arms.items():
        for arc in arcs:
            for frame in range(len(arc)-1):
                distance[arm].append(np.sqrt(((arc[frame+1][0]-arc[frame][0])**2)+(arc[frame+1][1]-arc[frame][1])**2))
...
def results(zones_order,results_array,start_frame,end_frame,conditions_folder):
    ...
    frac_in_arms = {'frac_in_openArms':frac_in_openArms, 'frac_in_closedArms':frac_in_closedArms, 'frac_in_closedAndMiddle':frac_in_closedAndMiddle, 'frac_in_middle':frac_in_middle}
    ...
    xticks(range(1800,results_array.shape[0],1800), [str(y) for y in range(1,int(floor(results_array.shape[0]/float(30*60)))+1)])
    xlabel('minutes')
...
def saveResults(conditions_folder,results_array,frac_in_arms,arm_entries,tot_arm_entries,frames_in_arms,total_distance,total_smoothed_distance,median_speed,smoothed_median_speed):
