...
TESTING_RUN__SAVE_GRAPHS = False  # If this is set to true, we will save histograms for eda
ZONES = None
...
def load_data(conditions_folder_path, start_frame=None, end_frame=None, ext='.zones.dict'):
    """
    INPUT: This function takes a conditions_folder (string), e.g. '20130202_163641_EPM_BWPOF2_1403_F/eda/900-7725_man-thresh-0.250',
...
def calculateSafetyFeatures(centroidsByArm, mouseLength):
    ...
    for arm in CLOSED_ARMS:
        ...
        # For eda we graph how far the mouse is from the end. We want to find the
...
def calculateRestFeatures(distancesPerArm):
    ...
    for arm in GOOD_ARMS:
        ...
        # For eda we graph the speeds of the mouse to see where cutoff should be for 'at rest'
...
def calculateArmEntries(zones_order, results_array, start_frame, end_frame, conditions_folder):
    ...
    fraction_in_arms = {
        'open': fractionInZones['OB'] + fractionInZones['OT'],
...
def calculateMouseLength(boundaries, distances):
    ...
    # This is for eda to figure out which percentile to use
