...
def find_ground_over_frames_matrix(frames,start_ground_anchors,end_ground_anchors,ahead_window=10,ahead_avg=5,fix_deviation=0.5,allow_passes=100,max_step_limit=None):
    ...
    fit_l = [rec_max(frames_deriv_Z[0][:,x],anchor,max_step_limit) for x,anchor in enumerate(
        Util.smooth(l, 5, interpolate_nones=True))]
    fit_surface = [fit_l]
    ...
    end_fit_l = [rec_max(frames_deriv_Z[-1][:,x],anchor,max_step_limit) for x,anchor in enumerate(
        Util.smooth(el, 5, interpolate_nones=True))]
    ...
    for iter,(fit_l,m) in enumerate(zip(fit_surface,frames_deriv_Z)):
        ...
        filt_fit_l = [rec_max(m[:,x],anchor,max_step_limit) for x,anchor in enumerate(
            Util.smooth(this_l, 5, interpolate_nones=True))]
...
def find_burrow(images,mask=None,change_zcut=3,smoothby=5,depress_groundmask=10,offsets=None,pct_to_average=0.01,frames_to_average=100,**ground_args):
    ...
    groundmask = mask_from_vector(
        Util.smooth(find_ground(first, mask, **ground_args), 10, interpolate_nones=True) + depress_groundmask, first.shape)
...
def calculate_cumulative_activity(analysis_dir,zcut,suppress_ground=40,shape=(480,720),be=None,write_files=True,force_all=False,xybounds=None,use_ground=None,preshrink=2):
    ...
    for f in actfiles:
        ...
        # this serves to place the most recent terminus at pt_flat[-1] for subsequent eda
...
def chain_outlines_from_mask_old(mask,origin=(0,0),grow_by=0,preshrink=2,xybounds=None,border=50):
    ...
    #added 20100508 to address "hang" on eda at edges of matrix
    if border > 0:
        ydim,xdim = mask.shape
